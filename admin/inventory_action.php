...
// Deduct stock (e.g., when order is completed – admin may also have this ability)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['deduct_stock'])) {
    ...
    if ($quantity > 0) {
        ...
        // Log the deduction (using admin_id)
        $log = mysqli_prepare($conn, "INSERT INTO inventory_logs (item_id, order_id, admin_id, quantity) VALUES (?, ?, ?, ?)");
        mysqli_stmt_bind_param($log, "iiid", $item_id, $order_id, $admin_id, $quantity);
        mysqli_stmt_execute($log);

        // Optionally update order status if order_id provided
        if ($order_id) {
